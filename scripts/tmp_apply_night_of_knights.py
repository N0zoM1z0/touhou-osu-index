...
# Two pre-existing title-family candidates need separate handling.
target = catalog.entries[154426]
unresolved = catalog.entries[1561908]
base_target = base[154426]
base_unresolved = base[1561908]
assert target.to_dict() == base_target, "154426 unexpectedly changed before final boundary review"
assert unresolved.to_dict() == base_unresolved, "1561908 unexpectedly changed before final boundary review"
assert target.confidence == "candidate"
...
assert not any(x in unresolved.evidence for x in ("manual:verified", "manual:excluded"))
...
# Minimal mutation: preserve every substantive metadata field from main.
...
assert unresolved.to_dict() == base_unresolved
...
No confidence decision is changed by this normalization except the separately reviewed `154426` candidate upgrade described above.
